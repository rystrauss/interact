"""Contains a dummy implementation of a parallel environment that actually executes environments sequentially.
...
from interact.common.parallel_env import ParallelEnv
...
class DummyParallelEnv(ParallelEnv):
    """A "parallel" env that runs multiple environments sequentially.

    That is, the step and reset commands are sent to one environment at a time.
    ...
    def step_async(self, actions):
        ...
        if not listify:
            ...
            assert self.num_envs == 1, f'actions {actions} is either not a list or has a wrong ' \
                                       f'size - cannot match to {self.num_envs} environments'
            self.actions = [actions]
