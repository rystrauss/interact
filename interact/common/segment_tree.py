...
class SegmentTree:
    """A segment tree data structure.
     ...
     A segment tree is useful for storing information about intervals, or segments.

    This class be used as regular array, but with two important differences:
    ...
    Args:
        ...
        operation (lambda obj, obj -> obj): operation for combining
            elements (eg. sum, max) must form a mathematical group together
            with the set of possible values for array elements
            (i.e. be associative)
        neutral_element (object): neutral element for the operation above.
    ...
    def __init__(self, capacity, operation, neutral_element):
        if not (capacity > 0 and capacity & (capacity - 1) == 0):
            raise ValueError('Capacity must be positive and a power of 2.')
...
class SumSegmentTree(SegmentTree):
    """A segment tree that uses the addition operator."""
    ...
    def sum(self, start=0, end=None):
        """Alias for the reduce method."""
        return self.reduce(start, end)

    def find_prefixsum_idx(self, prefixsum):
        """Find the highest index `i` in the array such that sum(arr[0] + arr[1] + ... + arr[i - i]) <= prefixsum.

        If array values are probabilities, this function allows you to sample indexes according to the
        discrete probability efficiently.
...
class MinSegmentTree(SegmentTree):
    """A segment tree that uses the min operator."""
